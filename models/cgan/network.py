...
class Generator(nn.Module):
    def __init__(self, noise_dim, out_img_size, hidden_dim=512):
...
class Conditional_Discriminator(nn.Module):
    def __init__(self, input_dim, condition_dim, hidden_dim = 512):
        super(Conditional_Discriminator, self).__init__()

        self.linear1    = nn.Linear(input_dim + condition_dim,  hidden_dim)
        self.linear2    = nn.Linear(hidden_dim, hidden_dim)
        self.linear3    = nn.Linear(hidden_dim, hidden_dim)
        self.linear4    = nn.Linear(hidden_dim, 1)
        self.leaky_relu = nn.LeakyReLU(0.2)
...
class Conditional_Generator(nn.Module):
    def __init__(self, noise_dim, condition_dim, out_img_size, hidden_dim=512):
        super(Conditional_Generator, self).__init__()
        self.linear1    = nn.Linear(noise_dim + condition_dim,  hidden_dim)
        self.linear2    = nn.Linear(hidden_dim, hidden_dim)
        self.linear3    = nn.Linear(hidden_dim, out_img_size)
        self.relu       = nn.ReLU()
        self.tanh       = nn.Tanh()
